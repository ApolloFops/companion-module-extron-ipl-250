...
- **Pulse Relay** - Pulses the relay at the selected port for the amount of time entered. This will toggle the relay twice, so if it starts off, it will go off->on->off, or if it starts on, it will go on->off->on.
